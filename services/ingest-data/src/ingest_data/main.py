...
from ingest_data.api import router
from ingest_data.deps import IngestDeps, build_deps, teardown_deps
from ingest_data.settings import Settings

_log = get_logger("ingest_data.main")
    ...
    if deps is None:
        ...
        async def lifespan(app: FastAPI) -> AsyncIterator[None]:
            ...
            _log.info(
                "ingest_data.started",
                env=settings.env,
                dns_resolver_id=settings.dns_resolver_id,
                ipdr_collector_id=settings.ipdr_collector_id,
            )
            try:
                yield
            finally:
                _log.info("ingest_data.stopping")
        ...
        app = FastAPI(title="ingest-data", version="0.1.0", docs_url="/docs", redoc_url=None)
        app.state.deps = deps
