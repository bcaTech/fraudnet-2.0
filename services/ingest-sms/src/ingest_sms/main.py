...
from ingest_sms.api import router
from ingest_sms.deps import IngestDeps, build_deps, teardown_deps
from ingest_sms.settings import Settings

_log = get_logger("ingest_sms.main")
    ...
    if deps is None:
        ...
        async def lifespan(app: FastAPI) -> AsyncIterator[None]:
            ...
            _log.info(
                "ingest_sms.started",
                env=settings.env,
                allow_body_capture=settings.allow_body_capture,
            )
            try:
                yield
            finally:
                _log.info("ingest_sms.stopping")
        ...
        app = FastAPI(title="ingest-sms", version="0.1.0", docs_url="/docs", redoc_url=None)
        app.state.deps = deps
